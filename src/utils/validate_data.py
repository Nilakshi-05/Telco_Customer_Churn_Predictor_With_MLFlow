...
def validate_data(df) -> Tuple[bool, List[str]]:
    ...
    This function implements critical data quality checks that must pass before modek training.
    ...
    ge_df.expect_columns_values_to_not_be_null("customerID")

    # Core demographic features
    ge_df.expect_columns_to_exist("gender")
    ge_df.expect_columns_to_exist("Partner")
    ge_df.expect_columns_to_exist("Dependants")

    # Service features (critical for churn analysis)
    ge_df.expect_columns_to_exist("PhoneService")
    ge_df.expect_columns_to_exist("InternetService") 
    ge_df.expect_columns_to_exist("Contract")

    # Financial features (key churn predictors)
    ge_df.expect_columns_to_exist("tenure")
    ge_df.expect_columns_to_exist("MonthlyCharges")
    ge_df.expect_columns_to_exist("TotalCharges")
    ...
    ge_df.expect_column_values_to_be_in_set("Dependants", ["Yes", "No"])
    ...
    ge_df.expect_column_values_to_be_in_set("InternetService", ["DSL", "Fiber optics", "No"])
    ...
    ge_df.expect_columns_values_to_not_be_null("tenure")
    ge_df.expect_columns_values_to_not_be_null("MonthlyCharges")
    ...
    ge_df.expect_column_pair_values_A_to_be_greater_than_B(
        ...
        or_equal = True,
        mostly = 0.95   # Allow 5% exceptions for new customers
    )
